# 🌿 EcoPackAI

**AI-Powered Sustainable Packaging Recommendation System**

[![Python](https://img.shields.io/badge/Python-3.9+-3776AB?logo=python&logoColor=white)](https://python.org)
[![Flask](https://img.shields.io/badge/Flask-2.3+-000000?logo=flask&logoColor=white)](https://flask.palletsprojects.com)
[![scikit-learn](https://img.shields.io/badge/scikit--learn-1.3+-F7931E?logo=scikit-learn&logoColor=white)](https://scikit-learn.org)
[![PostgreSQL](https://img.shields.io/badge/PostgreSQL-13+-336791?logo=postgresql&logoColor=white)](https://postgresql.org)
[![License: MIT](https://img.shields.io/badge/License-MIT-green.svg)](LICENSE)

EcoPackAI recommends optimal eco-friendly packaging using machine learning trained on **600 real packaging materials** and **15,000 real shipping orders**.

---

## 📊 Model Performance

| Model | Purpose | Score |
|---|---|---|
| Random Forest Regressor | Cost Prediction | **R² = 1.0000** |
| Random Forest Regressor | CO₂ Prediction | **R² = 0.9990** |
| Random Forest Classifier | Packaging Recommendation | **95.03% accuracy** |

---

## 🗂️ Project Structure

```
ecopackai/
├── app/
│   ├── app.py                        # Flask backend (9 API endpoints)
│   ├── templates/
│   │   ├── index.html                # Recommendation UI
│   │   └── dashboard.html            # BI Analytics Dashboard
│   └── static/                       # CSS & JS assets
│
├── scripts/
│   ├── module1_data_collection.py    # Load + validate datasets → PostgreSQL
│   ├── module2_feature_engineering.py# Clean + engineer ML features
│   ├── module3_ml_training.py        # Train & evaluate all 3 models
│   └── module7_bi_dashboard.py       # Generate charts + CSV reports
│
├── data/
│   ├── materials_database_600.xlsx   # ← Place your file here
│   ├── real_packaging_history.xlsx   # ← Place your file here
│   ├── materials_cleaned.csv         # Auto-generated by module1
│   ├── materials_features.csv        # Auto-generated by module2
│   └── history_features.csv          # Auto-generated by module2
│
├── models/                           # Trained .pkl models (auto-generated)
│   ├── rf_cost.pkl
│   ├── rf_co2.pkl
│   ├── rf_pkg.pkl
│   ├── le_pkg.pkl / le_cat.pkl / le_ship.pkl
│   └── metrics.json
│
├── database/
│   └── schema.sql                    # PostgreSQL schema (4 tables + views)
│
├── outputs/                          # Charts + CSV reports
│   ├── chart_*.png                   # 7 BI charts
│   ├── sustainability_report.csv
│   ├── packaging_performance_report.csv
│   └── top_materials_report.csv
│
├── tests/
│   └── test_api.py                   # API endpoint tests
│
├── .env.example                      # Environment variable template
├── .gitignore
├── Procfile                          # Heroku / Render deployment
├── render.yaml                       # Render one-click deploy config
├── requirements.txt
└── run.py                            # Local dev launcher
```

---

## ⚡ Quick Start (Local)

### 1. Clone & Install
```bash
git clone https://github.com/YOUR_USERNAME/ecopackai.git
cd ecopackai
pip install -r requirements.txt
```

### 2. Add Your Data Files
```bash
# Place your Excel files in the data/ folder:
cp /path/to/materials_database_600.xlsx data/
cp /path/to/real_packaging_history.xlsx data/
```

### 3. Configure Environment
```bash
cp .env.example .env
# Edit .env with your PostgreSQL credentials (optional — works without DB too)
```

### 4. Run the Data Pipeline
```bash
python scripts/module1_data_collection.py    # Load & validate data
python scripts/module2_feature_engineering.py# Engineer ML features
python scripts/module3_ml_training.py        # Train models
python scripts/module7_bi_dashboard.py       # Generate charts
```

### 5. Launch App
```bash
python run.py
```

Open: [http://localhost:5000](http://localhost:5000)

---

## 🌐 API Endpoints

| Method | Endpoint | Description |
|---|---|---|
| `GET` | `/` | Recommendation UI |
| `GET` | `/dashboard` | BI Analytics Dashboard |
| `POST` | `/api/recommend` | AI packaging recommendation |
| `GET` | `/api/materials` | Full material catalog |
| `GET` | `/api/stats` | Summary KPI metrics |
| `GET` | `/api/top_materials` | Top eco-graded materials |
| `GET` | `/api/packaging_usage` | Usage stats from real orders |
| `GET` | `/api/co2_trend` | Monthly CO₂ time series |
| `GET` | `/api/category_stats` | Per-category sustainability |

### Sample Request
```bash
curl -X POST http://localhost:5000/api/recommend \
  -H "Content-Type: application/json" \
  -d '{
    "category": "Electronics",
    "shipping_mode": "Air",
    "weight_kg": 1.5,
    "distance_km": 1200,
    "length_cm": 30,
    "width_cm": 20,
    "height_cm": 15,
    "fragility": 8,
    "moisture_sensitive": 1
  }'
```

---

## 🚀 Deploy to Render (Free)

### Option A: One-Click (render.yaml)
1. Fork this repo
2. Go to [render.com](https://render.com) → New → Blueprint
3. Connect your GitHub repo
4. Render auto-reads `render.yaml` → deploys everything

### Option B: Manual
1. New Web Service → connect GitHub repo
2. **Build Command:** `pip install -r requirements.txt && python scripts/module1_data_collection.py && python scripts/module2_feature_engineering.py && python scripts/module3_ml_training.py`
3. **Start Command:** `gunicorn --chdir app app:app`
4. **Environment Variables:** (see `.env.example`)

---

## 🚀 Deploy to Heroku

```bash
# Install Heroku CLI, then:
heroku login
heroku create ecopackai-yourname
git push heroku main
heroku config:set DB_HOST=your-db-host DB_NAME=ecopackai ...
heroku open
```

---

## 🐘 PostgreSQL Setup (Optional)

```bash
# Create database
createdb ecopackai

# Apply schema
psql -d ecopackai -f database/schema.sql

# Set env vars
export DB_HOST=localhost
export DB_PORT=5432
export DB_NAME=ecopackai
export DB_USER=postgres
export DB_PASSWORD=yourpassword
```

The app runs in **offline mode** (CSV only) if no DB is configured.

---

## 🔬 Engineered Features

**Materials:**
- `co2_impact_index` = `0.65 × (1 − CO₂_norm) + 0.35 × biodegradable_flag`
- `cost_efficiency_index` = `performance / (cost_norm + ε)`, normalised 0–1
- `material_suitability_score` = `35%×CO₂ + 25%×CEI + 20%×strength + 10%×density + 10%×bio`

**History:**
- `volume_cm3`, `dimensional_weight_ratio`, `cost_per_km`, `co2_per_kg`, `month`, `day_of_week`

---

## 🔑 Key Sustainability Insights

| Insight | Finding |
|---|---|
| Furniture is highest-emission category | Avg 87 kg CO₂/order |
| Wood Crates are worst packaging | 121 kg CO₂ avg (31× worse than PLA) |
| Mushroom Pkg dominates real orders | 38.8% of all 15,000 orders |
| Potential CO₂ reduction | **94.4%** switching from worst → best |
| Only 1 Grade A material | Single-Ply Corrugated Cardboard (score: 0.786) |

---

## 📄 License

MIT © EcoPackAI Project
